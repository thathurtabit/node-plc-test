...
function connected(err) {
  if (typeof err !== "undefined") {
    ...
    console.log(
      chalk.red(
        `
Error occurred while trying to connect to PLC - is it reachable? 😥
        `
      )
    );
    console.log(err);
...
function valuesReady(anythingBad, values) {
  if (anythingBad) {
    console.log(chalk.red("Error occurred while READING 👀 values 😥"));
  }
...
function valuesWritten(anythingBad) {
  if (anythingBad) {
    console.log(chalk.red("Error occurred while WRITING ✏️ values 😥"));
  }
